...
# Reads GitHub location of the fork and desired install location from 'summaWorkflow_public/0a_control_files/control_EastRiver.txt'

# --- Settings
# Find the line with the GitHub url
setting_line=$(grep -m 1 "^github_summa" ./0a_control_files/control_EastRiver.txt) # -m 1 ensures we only return the top-most result. This is needed because variable names are sometimes used in comments in later lines
...
dest_line=$(grep -m 1 "^install_path_summa" ./0a_control_files/control_EastRiver.txt) 
...
if [ "$summa_path" = "default" ]; then
 ...
 # Get the root path
 root_line=$(grep -m 1 "^root_path" ./0a_control_files/control_EastRiver.txt)
...
git clone --single-branch --branch master "$github_url" "$summa_path"
